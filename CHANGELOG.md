...
- Added the following machine types:
  - Fusion Ascent 12 (`ascent12`)
  - Fusion Ascent 24 (`ascent24`)
  - Fusion Ascent 36 (`ascent36`)
- Absolute units are now available for some process settings.
  - **[IMPORTANT]** As of firmware 1.1.4.0, there are known bugs with this that will prevent jobs from working as expected. These issues will be resolved in future firmware releases.
  - Speed can now be specified in absolute units of `mm/s`, `cm/s`, or `in/s`.
  - Frequency can now be specified in absolute units of `Hz`, `kHz`, or `MHz`.
  - Waveform can now be specified in absolute units of `ns`.

### Fixed

- `grayscale_3d` dithering now throws a validation error when used with fiber laser machines or processes because 3D jobs are not supported when using fiber lasers.
